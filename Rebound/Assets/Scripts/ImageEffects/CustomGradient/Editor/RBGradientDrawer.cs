...
[CustomPropertyDrawer(typeof(RBCustomGradient))]
public class RBGradientDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        RBCustomGradient gradient = GetDataObject(property) as RBCustomGradient;
        if (gradient == null)
        {
            return;
        }
        ...
        else
        {
            if (guiEvent.type == EventType.MouseDown && guiEvent.button == 0)
            {
                ...
                {
                    RBGradientEditor window = EditorWindow.GetWindow<RBGradientEditor>();
